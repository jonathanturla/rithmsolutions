...
5. Write a function addSpeaker to add a speaker to the array of speakers. The speaker you add must be an object with a key of name and a 
```
value of whatever you'd like.

function addSpeaker(){
	nestedObject.speakers.push({name: "Jon"});
}
```
...
function addLanguage(){
	nestedObject.data.languages.filipino = {hello: "kumusta"};
}
```
...
```
function addCountry(){
	nestedObject.data.continents.europe.countries.russia = {{capital: "Moscow"},{population:"144000000"}};
}
```
